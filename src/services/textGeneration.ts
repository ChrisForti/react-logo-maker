...
export class TextGenerationService {
    ...
    for (const component of components) {
      ...
      if (newLength <= this.MAX_LENGTH) {
        if (component.priority === 0) {
          ...
          const textReplacement = components.find(
            (c) => c.name === "textReplacement",
          );
          if (textReplacement) {
            assembledPrompt = this.applyTextReplacements(
              component.content,
              textReplacement,
            );
          } else {
        ...
        console.warn(
          `⚠️ Skipping component '${component.name}' - would exceed length limit`,
        );
      }
    }
  ...
  private buildTextReplacementComponent(
    settings: LogoSettings,
  ...
  private buildSpellingComponent(
    settings: LogoSettings,
  ): PromptComponent | null {
    if (!settings.textOverlay?.trim()) return null;
  ...
  private buildCleanupComponent(
    _settings: LogoSettings,
  ): PromptComponent | null {
    const content =
      ". Clean design, no small text, no decorative words, no border text.";
    return {
      name: "cleanup",
      priority: 6,
      content,
  ...
  public getPromptLengthEstimate(
    basePrompt: string,
    settings?: LogoSettings,
  ): number {
