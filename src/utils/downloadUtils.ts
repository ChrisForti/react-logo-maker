 ...
 */
async function downloadFileDesktop(blob: Blob, filename: string): Promise<void> {
...
function openInNewTab(dataUrl: string, filename: string): void {
  ...
  if (newWindow) {
    ...
    alert('Please allow popups to download the file. Alternatively, right-click the logo and select "Save Image As..."');
  }
}
...
export async function downloadPNG(
  ...
  try {
    ...
    // Convert canvas to blob (now using async version)
